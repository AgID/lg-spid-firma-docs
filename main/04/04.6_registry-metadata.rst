 ...
 * L'elemento :code:`<FileTransferService>` (*namespace* ``spid``) dotato dei
   seguenti attributi obbligatori:
    ...
    b. :code:`type` (facoltativo) — identificativo del protocollo, metodo o schema
       di comunicazione adottato –se previsto dal sistema di cui al §\ :ref:`5.2 <§5.2>`\ –
       valorizzato come un UUID v4 (come da :RFC:`4122`), ovvero da una stringa
       composta da un minimo di 3 e un massimo di 16 caratteri ASCII scelti nel
